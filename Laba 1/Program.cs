    ...
    class Program
    {
        ...
        static void ViewTable()
        {
            ...
            for(int j = 0; j < data.Count; j++)
            {
                output += (j + 1).ToString();
                output += " | ";
                output += data[j].film.PadRight(maxLength[0]);
                output += " | ";
                output += data[j].producer.PadRight(maxLength[1]);
                output += " | ";
                output += data[j].year.ToString().PadRight(maxLength[2]);
                output += " | ";
                output += types[((int)data[j].type)].PadRight(maxLength[3]);
                output += " |";
